...
`trades\_sha256` (`d9445e0d...`). Candle file SHA-256 recorded alongside,
...
\*\*What a legitimate Phase 48 would require.\*\* With 106 holdout trades and

materially negative expectancy, the burden of proof has shifted heavily.

A successor phase must not be framed as fixing Phase 47. It must test a

genuinely different hypothesis — for example intrabar stop execution

(a question about the measurement apparatus, not the strategy),

volatility expansion without a directional assumption (testing the

well-supported magnitude half while dropping the unsupported directional

half), or a market-structure filter on when breaks persist. The

distinguishing test: does the proposal originate in a REASON stated

independently of this result, or in the RESULT itself? Proposals of the

second kind are forbidden regardless of how they are worded.
